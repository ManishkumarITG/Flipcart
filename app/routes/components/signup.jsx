...
export default function Signup() {
  ...
  // "form" = enter details, "otp" = verify the emailed code
  ...
  // Step 1 -> request an OTP email, then move to the verify step.
  const requestOtp = async () => {
    setResponseData(null);
    setIsSubmitting(true);
    try {
      const res = await apiClass.sendSignupOtp(formData);
      setResponseData(res);
      if (res?.success) {
        setStep("otp");
        setOtp("");
      ...
      setResponseData({ success: false, message: "Could not send OTP. Please try again." });
    } finally {
  ...
  // Step 2 -> verify the OTP; account is only created on success.
  const submitOtp = async (e) => {
    e.preventDefault();
    if (!/^\d{6}$/.test(otp)) {
      setResponseData({ success: false, message: "Enter the 6-digit OTP sent to your email." });
      return;
    }
    ...
    setIsSubmitting(true);
    try {
      const res = await apiClass.sign_up({ email: formData.email, otp });
      setResponseData(res);
      if (res?.success) {
        navigate("/");
      }
    } catch (error) {
      setResponseData({ success: false, message: "Verification failed. Please try again." });
    } finally {
      setIsSubmitting(false);
    }
  };
            ...
            <form onSubmit={submitOtp} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  maxLength={6}
                  className="w-full px-4 py-2.5 tracking-[0.5em] text-center text-lg border border-gray-300 rounded focus:ring-2 focus:ring-flipkart-blue focus:border-flipkart-blue outline-none"
                  autoFocus
                />
                <p className="text-xs text-gray-500 mt-1">
                  We emailed a 6-digit code to {formData.email}. It expires in 5 minutes.
                </p>
              </div>
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <button
                  ...
                  className="text-flipkart-blue font-medium hover:underline disabled:text-gray-400 disabled:no-underline"
                >
